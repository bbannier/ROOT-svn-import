...
//-------------------------------------------------------------------------------
Reflex::Type
Reflex::TypeBuilder(const Reflex::Dictionary& dictionary,
                    const char* n,
                                              unsigned int modifiers ) {
//-------------------------------------------------------------------------------
// Construct the type information for a type.
   Reflex::Instance initialize_reflex;
   const Type& ret = Type::ByName(n, dictionary);

   if (ret.Id()) {
      return Type(ret, modifiers);
   } else {
      TypeName* tname = new TypeName(Names::FromDictionary(dictionary), n, 0);
      std::string sname = Tools::GetScopeName(n);

      if (!Scope::ByName(sname, dictionary).Id()) {
         Type scopeType = Type::ByName(sname, dictionary);
         if (scopeType.Id()) {
            TypeName* scopeTypeName = (TypeName*) scopeType.Id();
            if (scopeTypeName->LiteralName().IsLiteral()) {
               new ScopeName(Names::FromDictionary(dictionary), Literal(scopeTypeName->Name()), 0);
            } else {
               new ScopeName(Names::FromDictionary(dictionary), sname.c_str(), 0);
            }
         } else {
            new ScopeName(Names::FromDictionary(dictionary), sname.c_str(), 0);
         }
...
//-------------------------------------------------------------------------------
Reflex::Type
Reflex::ConstBuilder(const Reflex::Dictionary& dictionary,
                     const Type& t) {
//-------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------
Reflex::Type
Reflex::VolatileBuilder(const Reflex::Dictionary& dictionary,
                        const Type& t) {
//-------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------
Reflex::Type
Reflex::PointerBuilder(const Reflex::Dictionary& dictionary,
                       const Type& t,
                                                 const std::type_info & ti ) {
//-------------------------------------------------------------------------------
// Construct a pointer type.
   const Type & ret = Type::ByName( Pointer::BuildTypeName(t), dictionary);

   if (ret) {
      return ret;
   } else { return (new Pointer(dictionary, t, ti))->ThisType(); }
}


//-------------------------------------------------------------------------------
Reflex::Type
Reflex::PointerToMemberBuilder(const Reflex::Dictionary& dictionary,
                               const Type& t,
                                                         const Scope & s,
                                                         const std::type_info & ti ) {
//-------------------------------------------------------------------------------
// Construct a pointer type.
   const Type & ret = Type::ByName( PointerToMember::BuildTypeName(t,s), dictionary);

   if (ret) {
      return ret;
   } else { return (new PointerToMember(dictionary, t, s, ti))->ThisType(); }
}


//-------------------------------------------------------------------------------
Reflex::Type
Reflex::ReferenceBuilder(const Reflex::Dictionary& dictionary,
                         const Type& t) {
//-------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------
Reflex::Type
Reflex::ArrayBuilder(const Reflex::Dictionary& dictionary,
                     const Type& t,
                                               size_t n,
                                               const std::type_info & ti ) {
//-------------------------------------------------------------------------------
// Construct an array type.
   const Type & ret = Type::ByName(Array::BuildTypeName(t,n), dictionary);

   if (ret) {
      return ret;
   } else { return (new Array(dictionary, t, n, ti))->ThisType(); }
}


//-------------------------------------------------------------------------------
Reflex::Type
Reflex::EnumTypeBuilder(const Reflex::Dictionary& dictionary,
                        const char* nam,
                                                  const char * values,
                                                  const std::type_info & ti,
                                                  unsigned int modifiers ) {
...
// Construct an enum type.
   ...
   const Type& ret = Type::ByName(nam2, dictionary);
   ...
   Enum * e = new Enum(dictionary, nam, ti, modifiers );

   std::vector<std::string> valVec;
   Tools::StringSplit(valVec, values, ";");

   const Type& int_t = Type::ByName("int", dictionary);
...
//-------------------------------------------------------------------------------
Reflex::Type
Reflex::TypedefTypeBuilder(const Reflex::Dictionary& dictionary,
                           const char* nam,
                                                    const Type & t,
                           REPRESTYPE represType) {
//-------------------------------------------------------------------------------
// Construct a typedef type.
   Type ret = Type::ByName(nam, dictionary);
   ...
   return (new Typedef(dictionary, nam, t, Reflex::TYPEDEF, Reflex::Dummy::Type(), represType))->ThisType();
} // TypedefTypeBuilder


//-------------------------------------------------------------------------------
Reflex::Type
Reflex::FunctionTypeBuilder(const Reflex::Dictionary& dictionary,
                            const Type& r,
                                          const std::vector<Type> & p,
                                          const std::type_info & ti ) {
//-------------------------------------------------------------------------------
// Construct a function type.
   const Type& ret = Type::ByName(Function::BuildTypeName(r, p), dictionary);

   if ( ret && ret.TypeInfo() == ti ) {
      return ret;
   } else {
      return (new Function( dictionary, r, p, ti))->ThisType();
   }
}
...
// Construct a function type.
   std::vector< Type > v;
   const Type & ret = Type::ByName(Function::BuildTypeName(r,v), dictionary);

   if (ret) {
      return ret;
   } else { return (new Function(dictionary, r, v, typeid(UnknownType)))->ThisType(); }
}
