...
#---------------------------------------------------------------------------------
  def genClassDict(self, attrs, ioReadRules, ioReadRawRules):
    ...
    if self.isUnnamedType(clf) : 
      sc += '  ::Reflex::ClassBuilder(dictionary, Reflex::Literal("%s"), typeid(::Reflex::Unnamed%s), sizeof(%s), %s, %s)' % ( cls, self.xref[attrs['id']]['elem'], '__shadow__::'+ string.translate(str(clf),self.transtable), mod, typ )
    elif notAccessibleType :
      sc += '  ::Reflex::ClassBuilder(dictionary, Reflex::Literal("%s"), typeid(%s%s), sizeof(%s), %s, %s)' % ( cls, '::Reflex::' + self.xref[notAccessibleType]['attrs']['access'].title(), self.xref[attrs['id']]['elem'], '__shadow__::'+ string.translate(str(clf),self.transtable), mod, typ )
    else :
      typeidtype = '::' + cls
...
#----------------------------------------------------------------------------------
  def genAllTypes(self) :
    ...
    l = ['  ::Reflex::Type type_void = ::Reflex::TypeBuilder(dictionary, Reflex::Literal("void"));\n']
    d = ''
    for id in self.typeids :
      n = '  ::Reflex::Type type%s = ' % id
      if id[-1] == 'c':
        d += n + '::Reflex::ConstBuilder(dictionary, type'+id[:-1]+');\n'
      elif id[-1] == 'v':
        d += n + '::Reflex::VolatileBuilder(dictionary, type'+id[:-1]+');\n'
      else : 
        elem  = self.xref[id]['elem']
        attrs = self.xref[id]['attrs']
        if elem == 'PointerType' :
          d += n + '::Reflex::PointerBuilder(dictionary, type'+attrs['type']+');\n'
        elif elem == 'ReferenceType' :
          d += n + '::Reflex::ReferenceBuilder(dictionary, type'+attrs['type']+');\n'
        elif elem == 'ArrayType' :
          mx = attrs['max'].rstrip('u')
          # check if array is bound (max='fff...' for unbound arrays)
          if mx.isdigit() : alen = str(int(mx)+1)
          else            : alen = '0' 
          d += n + '::Reflex::ArrayBuilder(dictionary, type'+attrs['type']+', '+ alen +');\n'
        elif elem == 'Typedef' :
          sc = self.genTypeName(attrs['context'])
          if sc : sc += '::'
          d += n + '::Reflex::TypedefTypeBuilder(dictionary, Reflex::Literal("'+sc+attrs['name']+'"), type'+ attrs['type']+');\n'
        elif elem == 'OffsetType' :
          l.append(n + '::Reflex::TypeBuilder(dictionary, Reflex::Literal("%s"));\n' % self.genTypeName(attrs['id']))
        elif elem == 'FunctionType' :
          if 'returns' in attrs : d += n + '::Reflex::FunctionTypeBuilder(dictionary, type'+attrs['returns']
          else                  : d += n + '::Reflex::FunctionTypeBuilder(dictionary, type_void'
          args = self.xref[id]['subelems']
          ...
          l.append(n + '::Reflex::EnumTypeBuilder(dictionary, Reflex::Literal("' + sc + attrs['name'] + '"));\n')
        else :
         name = ''
         ...
         l.append(n + '::Reflex::TypeBuilder(dictionary, Reflex::Literal("'+name+'"));\n')
  ...
  def genNamespaces(self, selected ) :
    ...
    for ns in self.namespaces :
      if ns['id'] in used_context and 'name' in ns and  ns['name'] != '::' :
        s += '  ::Reflex::NamespaceBuilder nsb%d( dictionary, Reflex::Literal("%s") );\n' % (idx, self.genTypeName(ns['id']))
        idx += 1
    return s
#----------------------------------------------------------------------------------
...
#----------------------------------------------------------------------------------
  def genFunctions(self, selfunctions) :
    ...
    for f in selfunctions :
      ...
      s += '      ::Reflex::FunctionBuilder(dictionary, t%s, Reflex::Literal("%s"), function%s, 0, Reflex::Literal(%s), %s);\n' % (i, name, id, params, mod)
      i += 1;
    return s
#----------------------------------------------------------------------------------
  def genEnums(self, selenums) :
    ...
    for e in selenums :
      ...
      s += '      ::Reflex::EnumBuilder(dictionary, Reflex::Literal("%s"),typeid(%s), %s)' % (name, cname, mod)
      items = self.xref[id]['subelems']
      for item in items :
...
#----------------------------------------------------------------------------------
  def genVariables(self, selvars) :
    ...
    for v in selvars :
      ...
      s += '      ::Reflex::VariableBuilder(dictionary, Reflex::Literal("%s"), %s, (size_t)&%s, %s );\n' % (name, self.genTypeID(v['type']),self.genTypeName(id), mod)
    return s
 #----------------------------------------------------------------------------------
...
#----BasesMap stuff--------------------------------------------------------
  ...
  def genGetBasesTableDef( self, attrs, args ) :
    ...
    for b in bases :
      ...
      s += '    s_bases.push_back(std::make_pair(::Reflex::Base( ::Reflex::TypeBuilder(dictionary, Reflex::Literal("%s")), ::Reflex::BaseOffset< %s,%s >::Get(),%s), %d));\n' % (bname2, cl, bname, b[1], b[2])
