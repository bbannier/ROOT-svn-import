                    ...
                    : TVirtualPacketizer(input)
{
   ...
   if (TProof::GetParameter(input, "PROOF_LoadBasedSched",
                            loadBasedSched) == 0) {
      if (loadBasedSched == 1)
         fLoadBasedSched = kTRUE;
      else
         Info("Process",
            "The only accepted value of PROOF_LoadBasedSched parameter is 1 !");
   }
   ...
   if (fLoadBasedSched) {
      ...
      if (workers->GetSize() > 0)
         gProofServ->GetProof()->SetParallel(workers->GetSize());
      else {
...
void TPacketizerAdaptive::ValidateFiles(TDSet *dset, TList *slaves)
{
   ...
   while (kTRUE) {
      ...
      if ( entries > 0 ) {
         ...
         //
         //fValid = kFALSE; // all elements must be readable!
         if (gProofServ) {
