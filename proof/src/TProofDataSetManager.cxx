...
Bool_t TProofDataSetManager::ReadGroupConfig(const char *cf)
{
   ...
   if (cf && (strlen(cf) > 0)) {
      ...
      if (!TestPermissions(st, kReadPermission)) {
...
//______________________________________________________________________________
Bool_t TProofDataSetManager::TestPermissions(FileStat_t st, EAccessMode mode)
{
   ...
   // Ownership
   Bool_t o = (gSystem->GetUid() == st.fUid &&
               gSystem->GetGid() == st.fGid) ? kTRUE : kFALSE;
   Bool_t g = (gSystem->GetGid() == st.fGid) ? kTRUE : kFALSE;
