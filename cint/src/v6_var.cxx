...
static void G__class_2nd_decl_i(G__var_array* var, int ig15)
{
...
#ifdef G__ASM_DBG
   if (G__asm_dbg) {
      G__fprinterr(G__serr, "%3x: LD_VAR  %s index=%d paran=%d  %s:%d\n", G__asm_cp, var->varnamebuf[ig15], ig15, 0, __FILE__, __LINE__);
   }
#endif // G__ASM_DBG
   ...
   if (var->varlabel[ig15][1] /* number of elements */ || var->paran[ig15]) {
      ...
      pinc = var->varlabel[ig15][1] /* number of elements */;
#ifdef G__ASM_DBG
      if (G__asm_dbg) {
...
G__value G__letvariable(char* item, G__value expression, G__var_array* varglobal, G__var_array* varlocal)
{
   ...
   if (-1 != G__def_tagnum && G__def_struct_member) {
      ...
      // --
#ifdef G__ASM
      if (G__asm_noverflow && paran && (G__store_struct_offset != G__memberfunc_struct_offset || G__do_setmemfuncenv)) {
         // --
#ifdef G__ASM_DBG
         if (G__asm_dbg) {
   ...
   // Assign value.
   if (var && G__cintv6 && G__asm_noverflow && !G__asm_exec) {
      G__bc_assignment(var, ig15, paran, G__var_type, &result, G__struct_offset, store_struct_offset, para);
      return result;
   }
   if (var) {
      ...
      //
      //
      if ((tolower(var->type[ig15]) != 'u') && (result.type == 'u') && (result.tagnum != -1)) {
      ...
      if (
         !G__no_exec_compile &&
         var->varlabel[ig15][1] /* number of elements */ &&
         //(var->varlabel[ig15][1] /* number of elements */ != INT_MAX /* unspecified length array flag */) &&
         ...
         case 'c':
            // char
            {
               ...
               if (
                  ...
                  strcpy((char*) var->p[ig15], (const char*) result.obj.i);
...
G__value G__letstructmem(int store_var_type, char* varname, char* membername, char* tagname, G__var_array* varglobal, G__value expression, int objptr  /* 1: object, 2: pointer */)
{
   ...
   if (store_var_type == 'P') {
      std::sprintf(varname, "&%s", membername);
      std::strcpy(membername, varname);
   }
   if (store_var_type == 'v') {
      std::sprintf(varname, "*%s", membername);
      std::strcpy(membername, varname);
   ...
   if (islower(result.type) && (objptr == 2)) {
      ...
      if (flag) {
         ...
         G__tagnum = store_tagnum;
         G__store_struct_offset = store_struct_offset;
         if ((G__dispmsg >= G__DISPROOTSTRICT) || (G__ifile.filenum <= G__gettempfilenum())) {
         ...
         if (var->paran[ig15] <= paran) {
            ...
            if (G__decl) {
               ...
               if (ig2 == 0) {
                  ...
                  if (addr < 0) {
                     sprintf(result7, "operator=((%s)(%ld),%s)", G__fulltagname(var->p_tagtable[ig15], 1), addr, ttt);
                  }
                  else {
                     sprintf(result7, "operator=((%s)%ld,%s)", G__fulltagname(var->p_tagtable[ig15], 1), addr, ttt);
                  }
            ...
            if (ig2) {
               ...
               *result = para;
               ...
               if (result->tagnum == var->p_tagtable[ig15]) {
                  std::memcpy(
                        (void*)(G__struct_offset + var->p[ig15] + linear_index * G__struct.size[var->p_tagtable[ig15]]),
                        (void*)(G__int(*result)),
            ...
            if (var->p[ig15] != 0 && G__COMPILEDGLOBAL != var->statictype[ig15])
               free((void*)var->p[ig15]);
...
#ifdef G__ASM_DBG
               if (G__asm_dbg) {
                  G__fprinterr(G__serr, "%3x: PUSHSTROS  %s:%d\n", G__asm_cp - 2, __FILE__, __LINE__);
                  G__fprinterr(G__serr, "%3x: SETSTROS  %s:%d\n", G__asm_cp - 1, __FILE__, __LINE__);
               }
#endif // G__ASM_DBG
               G__asm_inst[G__asm_cp] = G__PUSHSTROS;
               G__asm_inst[G__asm_cp+1] = G__SETSTROS;
   ...
   if (!ig2) {
      ...
      if (pdest < 0)
         sprintf(result7, "operator=((%s)(%ld),%s)"
               , G__fulltagname(tagnum, 1), pdest, ttt);
      else
         sprintf(result7, "operator=((%s)%ld,%s)"
               , G__fulltagname(tagnum, 1), pdest, ttt);
      para = G__getfunction(result7, &ig2 , G__TRYNORMAL);
   ...
   if (islower(G__var_type)) { \
      ...
      if ( \
         ...
         if (var->varlabel[ig15][1] /* number of elements */ == INT_MAX /* unspecified length flag */) { \
            ...
            /* -- We are initializing a non-array variable. */ \
            *((CASTTYPE*) var->p[ig15]) = (CASTTYPE) CONVFUNC(result); \
         } \
      } \
   } \
   else { \
...
#define G__ALLOC_VAR_REF(SIZE, CASTTYPE, CONVFUNC) \
   if (islower(G__var_type)) { \
      /* -- Not a pointer. */ \
      var->p[ig15] = G__malloc(num_elements ? num_elements : 1, SIZE, item); \
      if ( \
         ( \
            /* Normal variable. */ \
            !G__def_struct_member || \
            /* Static class member, during parsing, not running. */ \
            (G__static_alloc && G__prerun) && \
         ) && \
         /* Init statics only during parsing, not running. */ \
         (!G__static_alloc || G__prerun) && \
         ( \
            /* Variable is not of class type or it is not pre-allocated. */ \
            (G__globalvarpointer == G__PVOID) || \
...
static G__value G__allocvariable(G__value result, G__value para[], G__var_array* varglobal, G__var_array* varlocal, int paran, int varhash, char* item, char* varname, int parameter00)
{
   ...
   int store_var_type = 0;
   if (G__automaticvar && ((G__var_type == 'p') && !G__definemacro) && (G__globalcomp == G__NOLINK)) {
      if (result.tagnum == -1) {
         ...
         if (std::isupper(result.type)) {
            ...
            if (result.tagnum == G__p_tempbuf->obj.tagnum && G__templevel == G__p_tempbuf->level) {
...
#endif // G__NEWINHERIT
            if (G__cintv6) {
               if (!G__const_noerror || (result.isconst & G__STATICCONST)) {
                  std::strcpy(varname, ttt);
               }
               else {
                  std::sprintf(varname, "_%s", ttt);
               }
            }
            else {
               std::strcpy(varname, ttt);
   ...
   //
   //  FIXME: This can be removed, we are never going to use G__cintv6 stuff.
   //
   ...
   #ifdef G__ASM_DBG
               if (G__asm_dbg) {
                  G__fprinterr(G__serr, "%3x: INIT_REF  index: %d paran: %d type: '%c' var: %08lx %s:%d\n", G__asm_cp, ig15, paran, G__var_type, (long) var, __FILE__, __LINE__);
               }
   #endif // G__ASM_DBG
   ...
   #ifdef G__ASM_DBG
            if (G__asm_dbg) {
               G__fprinterr(G__serr, "%3x: POP  %s:%d\n", G__asm_cp, __FILE__, __LINE__);
            }
   #endif // G__ASM_DBG
...
#ifdef G__ASM_DBG
               if (G__asm_dbg) {
                  G__fprinterr(G__serr, "%3x: CTOR_SETGVP %s index=%d paran=%d\n", G__asm_cp, item, ig15, paran);
               }
#endif // G__ASM_DBG
...
#ifdef G__ASM_DBG
               if (G__asm_dbg) {
                  G__fprinterr(G__serr, "%3x: LD_VAR  item: '%s' index: %d paran: %d type: '%c' var: %08lx  %s:%d\n", G__asm_cp, item, ig15, paran, 'p', (long) var, __FILE__, __LINE__);
               }
#endif // G__ASM_DBG
   ...
   switch (G__var_type) {
      case 'u':
         ...
         if (
            ...
            G__fprinterr(G__serr, "Error: '%s' Data member belongs to the abstract class '%s', Objetcts of an abstract class are not allowed", item, G__struct.name[G__tagnum]);
            G__genericerror(0);
      ...
      case 'U':
         ...
         if ((num_elements > 0) && (result.type != '\0')) {
            // char* argv[];
      ...
      case 'c':
         ...
         if ((G__var_type == 'c') && var->varlabel[ig15] /* number of elements */ && (result.type == 'C')) {
      ...
      case 'm':
         ...
         *((fpos_t*) var->p[ig15]) = *((std::fpos_t*) result.obj.i);
         break;
      case 'a':
         // pointer to member function
...
//______________________________________________________________________________
int G__asm_gen_stvar(long G__struct_offset, int ig15, int paran, G__var_array* var, char* item, long store_struct_offset, int var_type, G__value* presult)
{
   if (G__cintv6) {
   ...
   if (G__no_exec_compile) {
      switch (var->type[ig15]) {
         ...
         case 'U':
            if (ig25 >= paran) {
               G__letstructp(result, G__struct_offset, ig15, linear_index, var, paran, item, secondary_linear_index);
            }
            break;
      }
   }
...
//______________________________________________________________________________
G__value G__getvariable(char* item, int* known2, G__var_array* varglobal, G__var_array* varlocal)
{
   struct G__var_array* var = 0;
      ...
      case '*':
         ...
         if (
            ...
            *known2 = 1;
            ...
            if (store_var_type != 'p') {
               para[0] = G__toXvalue(para[0], store_var_type);
            }
            return para[0];
         }
         //
   ...
   double_quote = 0;
   paren = 0;
   // If struct member, let G__getstructmem do the work.
   if (flag) {
      result = G__getstructmem(store_var_type, varname, membername, tagname, known2, varglobal, flag);
      return result;
   }
   /************************************************************
    * !varglobal means, G__getvariable() is called from
    * G__getvariable() or G__letvariable() and G__store_struct_offset
    * is set by parent G__getvariable().
    *  This is done in different manner in case of loop
    * compilation execution.
   ...
   /************************************************************
    * Separate variable name by scanning char* item up to "[("
    *
    *   'varname[]'
    *           ^
    ************************************************************/
   ig15 = 0;
   varhash = 0;
   while ((ig15 < lenitem) && (item[ig15] != '(') && (item[ig15] != '[')) {
      varname[ig15] = item[ig15];
      varhash += item[ig15++];
   }
   /************************************************************
    ...
    ************************************************************/
   if (item[ig15] == '(') {
      // If we see 'func(xxxx)', then return, G__getitem() will handle it.
      return G__null;
   }
   /************************************************************
    * var[x](a,b);
   ...
   //
   // Evaluate array index expressions, if any.
   //
#ifdef G__ASM
   if (
      G__asm_noverflow &&
   ...
   if (var) {
      ...
      if (
         ...
         'p' != var->type[ig15] &&
         (
            ((var->constvar[ig15] & G__CONSTVAR) == 0) ||
         ...
         switch (G__var_type) {
            ...
            case 'P':
               if (std::islower(var->type[ig15])) {
                  result.type = std::toupper(var->type[ig15]);
               }
               else {
                  result.type = var->type[ig15];
                  switch (var->reftype[ig15]) {
...
//
...
#ifdef G__ASM_DBG
               if (G__asm_dbg) {
                  G__fprinterr(G__serr, "%3x: ADDSTROS %d  %s:%d\n", G__asm_cp, G__struct_offset - store_struct_offset, __FILE__, __LINE__);
               }
#endif // G__ASM_DBG
               G__asm_inst[G__asm_cp] = G__ADDSTROS;
               G__asm_inst[G__asm_cp+1] = G__struct_offset - store_struct_offset;
...
#ifdef G__ASM_DBG
            if (G__asm_dbg) {
               G__fprinterr(G__serr, "%3x: LD_MSTR  item: '%s' index: %d paran: %d type: '%c' var: %08lx  %s:%d\n", G__asm_cp, item, ig15, paran, G__var_type, (long) var, __FILE__, __LINE__);
            }
#endif
            G__asm_inst[G__asm_cp] = G__LD_MSTR;
            G__asm_inst[G__asm_cp+1] = ig15;
      ...
      if (
         ...
         *known2 = 1;
         ...
         G__var_type = 'p';
         if ('u' == std::tolower(var->type[ig15])) {
...
//
      ...
      linear_index = 0;
      {
         int tmp = var->varlabel[ig15][0] /* stride */;
         for (ig25 = 0; (ig25 < paran) && (ig25 < var->paran[ig15]); ++ig25) {
            linear_index += G__int(para[ig25]) * tmp;
            tmp /= var->varlabel[ig15][ig25+2];
         }
      }
      //fprintf(stderr, "G__get_variable: item: '%s'  calculated linear_index: %d\n", item, linear_index);
      secondary_linear_index = 0;
      {
         // -- Calculate secondary_linear_index.
         int tmp = var->varlabel[ig15][ig25+3];
      ...
      switch (var->type[ig15]) {
         ...
         case 'D': /* double */
            ...
            case 'v':
               ...
               switch (var->reftype[ig15]) {
                  case G__PARANORMAL:
                     ...
                     result.ref = (*(long*) (G__struct_offset + var->p[ig15] + (linear_index * G__LONGALLOC)));
                     if (result.ref) {
                        G__letdouble(&result, 'd', (double) (*((double*) result.ref)));
                     }
                     break;
                  case G__PARAP2P:
                     if (var->paran[ig15] < paran) {
                        address = G__struct_offset + var->p[ig15] + (linear_index * G__LONGALLOC);
                        result.ref = *(long*) (((double*) (*(long*) address)) + secondary_linear_index);
                        if (result.ref) {
                           G__letdouble(&result, 'd', (double)(*(double*)result.ref));
                        }
                     }
                        ...
                        result.ref = *(long*)(G__struct_offset + var->p[ig15] + (linear_index * G__LONGALLOC));
                        G__letint(&result, 'D', *(long*)(*(long*)(G__struct_offset + var->p[ig15] + (linear_index * G__LONGALLOC))));
                     }
                     break;
               }
               break;
            case 'P':
               /* -- Return a pointer to the pointer variable value.  MyType* var; MyType** v = &var; */
               if (var->paran[ig15] == paran) {
                  G__letint(&result, 'D', G__struct_offset + var->p[ig15] + (linear_index * G__LONGALLOC));
               }
               else if (var->paran[ig15] < paran) {
                  address = G__struct_offset + var->p[ig15] + (linear_index * G__LONGALLOC);
                  if (var->reftype[ig15] == G__PARANORMAL) {
                     G__letint(&result, 'D', (long)((double*)(*(long*)address) + secondary_linear_index));
                  }
                  else {
                     G__letint(&result, 'D', (long)((long*)(*(long*)address) + secondary_linear_index));
                     result.obj.reftype.reftype = G__PARAP2P;
                  }
               }
               else {
            ...
            default :
               ...
               if (paran == var->paran[ig15]) {
                  /* MyType* var[ddd]; MyType* v = var[xxx]; */
                  result.ref = (long)(G__struct_offset + var->p[ig15] + (linear_index * G__LONGALLOC));
                  G__letint(&result, 'D', *((long*) result.ref));
               }
               else if (paran > var->paran[ig15]) {
                  /* -- Pointer to array reimplementation. */
                  /* MyType* var[ddd];  v = var[xxx][yyy]; */
                  ...
                  if (var->reftype[ig15] == G__PARANORMAL) {
                     ...
                     /* Pointer to array reimplementation. */
                     result.ref = (long)((long*)(*(long*)address) + secondary_linear_index);
                     G__letint(&result, 'D', (long)((double*)(*((long*)result.ref))));
                     if (G__PARAP2P < var->reftype[ig15]) {
                        result.obj.reftype.reftype = var->reftype[ig15] - 1;
                     }
                  }
                     ...
                     switch (result.obj.reftype.reftype) {
                        case G__PARANORMAL:
                           result.ref = (long)((double*)(*((long*)result.ref)) + para[paran-1].obj.i);
                           G__letdouble(&result, 'd', *((double*)result.ref));
                           break;
                        case 1:
                           result.ref = (long)((long*)(*((long*)result.ref)) + para[paran-1].obj.i);
                           G__letint(&result, 'D', *((long*)result.ref));
         ...
         case 'u': /* struct, union */
            ...
            break;
         case 'U': /* class, struct, union, enum pointer */
            switch (G__var_type) {
   ...
   if (!done) {
      ...
      *known2 = G__getthis(&result, varname, item);
      if (*known2) {
         if (paran > 0) {
      ...
      if (isupper(var->type[ig15]) && (var->type[ig15] != 'P') && (var->type[ig15] != 'O')) {
         G__getpointer2pointer(&result, var, ig15, paran);
      }
...
#define G__OLDIMPLEMENTATION965 // FIXME: Remove this?
   ...
   G__isconst = result.isconst;
#endif // G__OLDIMPLEMENTATION1259
   if ((G__tagnum < 0) || (isupper(result.type) && (result.obj.reftype.reftype >= G__PARAP2P))) {
      if (membername[0] != '~') {
         if (!G__const_noerror) {
...
#ifdef G__ASM_DBG
         if (G__asm_dbg) {
            G__fprinterr(G__serr, "%3x: LD_THIS %c\n", G__asm_cp, G__var_type);
         }
#endif // G__ASM_DBG
...
struct G__var_array* G__searchvariable(char* varname, int varhash, G__var_array* varlocal, G__var_array* varglobal, long* pG__struct_offset, long* pstore_struct_offset, int* pig15, int isdecl)
{
   ...
   struct G__var_array* enclosing_scope = 0;
#endif
#ifdef G__ROOT
   ...
   while (ilg <= (G__GLOBAL + 1))
   {
      ...
      do {
         ...
         while (var) {
            ig15 = 0;
            while (ig15 < var->allvar) {
               //fprintf(stderr, "var: %08lx name: '%-8.8s' allvar: %d ig15: %d next: %08lx\n", var, var->varnamebuf[ig15], var->allvar, ig15, var->next);
               if (
