...
#if defined (_WIN32) 
SharedLibrary s_libInstance("libtest_Class2DictRflx.dll");
#else
SharedLibrary s_libInstance("libtest_Class2DictRflx.so");
#endif 
...
ReflexSimple2Test::loadLibrary() {
   ...
   if ( ! libLoaded ) std::cout << s_libInstance.Error() << std::endl;
...
ReflexSimple2Test::testFunctionMembers() {
  ...
  int c = (int)'?';
  m.Invoke(o, c);
  CPPUNIT_ASSERT_EQUAL('h',(char)c);

  m = t.MemberByName("g");
  CPPUNIT_ASSERT(m);
  m.Invoke(o, c);
  CPPUNIT_ASSERT_EQUAL('g',(char)c);
  ...
  CPPUNIT_ASSERT(m);
  m.Invoke(o, c);
  CPPUNIT_ASSERT_EQUAL('f',(char)c);

  m = t.MemberByName("d");
  CPPUNIT_ASSERT(m);
  m.Invoke(o, c);
  CPPUNIT_ASSERT_EQUAL('d',(char)c);

  m = t.MemberByName("b");
  CPPUNIT_ASSERT(m);
  m.Invoke(o, c);
  CPPUNIT_ASSERT_EQUAL('b',(char)c);
  
  m = t.MemberByName("a");
  CPPUNIT_ASSERT(m);
  m.Invoke(o, c);
  CPPUNIT_ASSERT_EQUAL('a',(char)c);
  
  m = t.MemberByName("m");
  CPPUNIT_ASSERT(m);
  m.Invoke(o, c);
  CPPUNIT_ASSERT_EQUAL('m',(char)c);

  m = t.MemberByName("e");
  CPPUNIT_ASSERT(m);
  m.Invoke(o, c);
  CPPUNIT_ASSERT_EQUAL('e',(char)c);

  m = t.MemberByName("c");
  CPPUNIT_ASSERT(m);
  m.Invoke(o, c);
  CPPUNIT_ASSERT_EQUAL('c',(char)c);
