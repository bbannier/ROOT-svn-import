...
Cint::G__DataMemberInfo::G__DataMemberInfo()
   : handle(0), index(0), belongingclass(0), type()
{
   Init();
}
...
void Cint::G__DataMemberInfo::Init(long handlein, long indexin, G__ClassInfo* belongingclassin)
{
   ...
   if (handlein) {
      ...
      if (belongingclassin && belongingclassin->IsValid()) {
         // The passed handlein was not to G__global.
...
long Cint::G__DataMemberInfo::Property()
{
   ...
   if (var->statictype[index] == G__LOCALSTATIC) {
      property |= G__BIT_ISSTATIC;
   }
   if (var->statictype[index] == G__USING_STATIC_VARIABLE) {
      property |= G__BIT_ISUSINGVARIABLE | G__BIT_ISSTATIC;
   }
   if (var->statictype[index] == G__USING_VARIABLE) {
      property |= G__BIT_ISUSINGVARIABLE;
   }
   if (var->reftype[index] == G__PARAREFERENCE) {
      property |= G__BIT_ISREFERENCE;
   }
   if (isupper(var->type[index])) {
      property |= G__BIT_ISPOINTER;
   }
   if (var->constvar[index] & G__CONSTVAR) {
      property |= G__BIT_ISCONSTANT;
   }
   if (var->constvar[index] & G__PCONSTVAR) {
      property |= G__BIT_ISPCONSTANT;
   }
   if (var->paran[index]) {
      property |= G__BIT_ISARRAY;
   }
   if (var->p_typetable[index] != -1) {
      property |= G__BIT_ISTYPEDEF;
   }
   if (var->p_tagtable[index] == -1) {
      property |= G__BIT_ISFUNDAMENTAL;
   }
   else {
      if (
         strcmp(G__struct.name[var->p_tagtable[index]], "G__longlong") == 0 ||
         strcmp(G__struct.name[var->p_tagtable[index]], "G__ulonglong") == 0 ||
         strcmp(G__struct.name[var->p_tagtable[index]], "G__longdouble") == 0
      ) {
         property |= G__BIT_ISFUNDAMENTAL;
         if (
            (var->p_typetable[index] != -1) &&
            (
...
void Cint::G__DataMemberInfo::SetGlobalcomp(G__SIGNEDCHAR_T globalcomp)
{
   if (IsValid()) {
      ...
      if (G__NOLINK == globalcomp) {
         var->access[index] = G__PRIVATE;
      }
      else {
         var->access[index] = G__PUBLIC;
      }
   }
}
...
int Cint::G__DataMemberInfo::Next()
{
   ...
   ++index;
   if (index >= var->allvar) {
      // Reached end of var page, move to next page.
      int t = var->tagnum; // FIXME: We shouldn't need to save this.
      var = var->next;
      if (!var) {
         // No more data members, go invalid.
         handle = 0;
         index = -1;
         return 0;
      }
      else {
         var->tagnum = t; // FIXME: Why overwrite here?
         handle = (long) var;
         index = 0;
      }
   }
