...
* G__include_file()
*
* Keyword '#include' is read in G__exec_statement(&brace_level);
...
#  else
      ...
      else                 sprintf(G__cintsysdir, "%s%scint7%sextsrc", env, G__psep, G__psep);
#  endif
# endif /* ROOTBUILD */
...
int Cint::Internal::G__isfilebusy(int ifn)
{
  ...
  *********************************************************************/
  ...
  for(::Reflex::Member_Iterator i1 = ifunc.FunctionMember_Begin();i1!=ifunc.FunctionMember_End();++i1) {
     if( 0!=G__get_funcproperties(*i1)->entry.busy && G__get_funcproperties(*i1)->filenum>=ifn ) {
        G__fprinterr(G__serr,"Function %s() busy. loaded after \"%s\"\n"
           ,i1->Name().c_str(),G__srcfile[ifn].filename);
        flag++;
     }
  }
...
static void G__checkIfOnlyFunction(int fentry)
{
  ...
  if(dictpos->var == var && dictpos->ig15 == var.DataMemberSize()) {
    ...
    while(var2 && (var2 != var || ig152 != var.DataMemberSize())) {
      if('p'!=G__get_type(var2.DataMemberAt(ig152).TypeOf())) { 
        varflag = 0;
        break;
      }
