...
int FourBin(bool doFeldmanCousins=false){
  ...
  // inspect workspace
  //  wspace->Print();
  wspace->pdf("model")->graphVizTree("FourBin_Frequentist.dot");
  wspace->factory("PROD::baysianModel(model,prior)"); // only for visualization
  wspace->pdf("baysianModel")->graphVizTree("FourBin_Bayesian.dot");
  ...
  // use BayesianCalculator (only 1-d)  
