...
void rs101b_limitexample()
{
  ...
  wspace->factory("Poisson::countingModel(obs[150,0,300], sum(s[50,0,120]*ratioSigEff[1.,0.8,1.5],b[100,0,300]*ratioBkgEff[1.,0.8,1.5]))"); // counting model
  wspace->factory("obsSigEff[1.,0.,3.]"); // observed sideband or auxiliary measurement
  wspace->factory("obsBkgEff[1.,0.,3.]"); // observed sideband or auxiliary measurement
  wspace->factory("Gaussian::sigConstraint(obsSigEff,ratioSigEff,0.05)"); // 5% signal efficiency uncertainty
  wspace->factory("Gaussian::bkgConstraint(obsBkgEff,ratioBkgEff,0.1)"); // 10% background efficiency uncertainty
  ...
  RooRealVar* obsBkgEff = wspace->var("obsSigEff"); // get the observable
  RooRealVar* obsSigEff = wspace->var("obsBkgEff"); // get the observable
  ...
  obs->setVal(160.);
  RooArgSet observables(*obs, *obsSigEff,*obsBkgEff);
  RooDataSet* data = new RooDataSet("exampleData", "exampleData", observables);
  ...
  RooRealVar* sPrime = (RooRealVar*) s->clone("sPrime");
  RooRealVar* ratioSigEffPrime = (RooRealVar*) ratioSigEff->clone("ratioSigEffPrime");
  RooRealVar* ratioBkgEffPrime = (RooRealVar*) ratioBkgEff->clone("ratioBkgEffPrime");
  TMatrixDSym covFit = fitResult->covarianceMatrix();
  RooMultiVarGaussian mvg("mvg","mvg",
			  RooArgList(*ratioBkgEff, *ratioSigEff, *s),
			  RooArgList(*ratioBkgEffPrime, *ratioSigEffPrime, *sPrime),
			  covFit );
  //  RooPolynomial uniform("uniform","",*s); 
  //  RooConstVar dummy("dummy","",0.9);
  //  RooAddPdf mixture("mixture","mixture",mvg,uniform, dummy);
  //  mixture.printCompactTree();
  PdfProposal pdfProposal(mvg);
  //  PdfProposal pdfProposal(mixture);
  pdfProposal.AddMapping(*sPrime, *s);
  pdfProposal.AddMapping(*ratioSigEffPrime, *ratioSigEff);
  pdfProposal.AddMapping(*ratioBkgEffPrime, *ratioBkgEff);
  pdfProposal.SetCacheSize(100);
  MCMCCalculator mc;
  mc.SetPdf(*modelWithConstraints);
  mc.SetData(*data);
  mc.SetParameters(paramOfInterest);
  mc.SetProposalFunction(pdfProposal);
...
/*
Profile upper limit on s = 88.0518
FC lower limit on s = 15
FC upper limit on s = 103.8
MCMC lower limit on s = 34.8
MCMC upper limit on s = 87.6
Real time 0:01:36, CP time 94.410
*/
