...
void rs201b_hybridcalculator(int ntoys = 1000)
{
  ...
  // The background rate is uncertain, but the sideband helps constrain it.
  //
  ...
  // used to generate the pseudo experiments. This is the equivalent of
  ...
  // However, if this is done the control measurement  should also
  ...
  int eventspertoy = 0; // use Extended()
  //int eventspertoy = 100;

  // M A K E   T O Y   D A T A
  RooDataSet* data = NULL;
  if(eventspertoy == 0) data = w.pdf("main_pdf")->generate(*w.set("obs"), RooFit::Extended());
  else                  data = w.pdf("main_pdf")->generate(*w.set("obs"), eventspertoy);
  data->Print();


  // C H O O S E   T E S T  S T A T I S T I C  F O R  T E S T
  // by default the hybrid calculator will use the ToyMCSampler
  // and the ratio of profiled likelihoods as a test statistic
  // but the user can have full control of the choice of test 
  // statistic and the method for sampling it.
  //
  // here we demonstrate the creation of several appropriate test statistics
  ...
  // Construct an importance density to be used for importance sampling.
  // The first option is a weighted mixture of the s+b pdf and the
  // b pdf. The s+b pdf part still depends on the parameters of
  // interest and therefore a snapshot containing the respective
  // value needs to be specified.
  // The second option is a marginalized pdf over the parameter
  // of interest with an importanceWeightFunction. This option is slow.
  //
  //
  // This is a quick and dirty way of making a copy of
  // main_pdf that has a different sig_yield which is
  // set to 0.0 and independent of the old sig_yield.
  // The new function is therefore called main_pdf0
  // and the new parameter used is sig_yield0.
  //
  // Also suppress INFO messages about members that are
  // already in workspace w.
  RooFit::MsgLevel msglevel = RooMsgService::instance().globalKillBelow();
  RooMsgService::instance().setGlobalKillBelow(RooFit::ERROR);
  w.import(*w.pdf("main_pdf"),
     RooFit::RenameVariable("main_pdf", "main_pdf0"),
     RooFit::RenameVariable("sig_yield", "sig_yield0"),
  ...
  // ProofConfig takes as arguments the workspace, the number of
  ...
  //  myH2.SetNullImportanceDensity(w.pdf("impDens"), w.set("poi"));

  // adaptive sampling (only with HybridCalculatorImp)
  //  myH2.SetNToysInTails(100,100);
