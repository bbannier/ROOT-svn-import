...
RooDataSet::NumberCountingData(x_0,y_0,x_1,y_1)
  */
  ...
  //  RooAbsData* data = ws2->data("NumberCountingData");

  ProfileLikelihoodCalculator plc;
  plc.SetWorkspace(ws2);
  plc.SetCommonPdf("joint");
  plc.SetData("NumberCountingData");

  //  RooArgSet* nullParams = new RooArgSet("nullParams",*(ws2->var("masterSignal")));
  RooRealVar* x = ws2->var("masterSignal");
  RooArgSet* nullParams = new RooArgSet("nullParams");
  nullParams->addClone(*x);

  // debugging
  //  ws2->var("masterSignal")->Print();
  //  ws2->var("b_0")->Print();
  //  ws2->var("b_1")->Print();
