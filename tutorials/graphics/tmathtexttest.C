void tmathtexttest(void)
{
	...
	l.SetTextAlign(23);
	l.SetTextSize(0.06);
	l.DrawLatex(0.5, 0.950, "#prod_{j#geq0} #left(#sum_{k#geq0} a_{jk}z^{k}#right) = #sum_{n#geq0} z^{n} #left(#sum_{k_{0},k_{1},#3dots#geq0} a_{0k_{0}}a_{1k_{1}} #3dots #right)");
	l.DrawLatex(0.5, 0.800, "W_{#delta_{1}#rho_{1}#sigma_{2}}^{3#beta} = U_{#delta_{1}#rho_{1}#sigma_{2}}^{3#beta} + #frac{1}{8#pi^{2}} #int_{#alpha_{1}}^{#alpha_{2}} d#alpha'_{2} #left[ #frac{U_{#delta_{1}#rho_{1}}^{2#beta} - #alpha'_{2} U_{#rho_{1}#sigma_{2}}^{1#beta}}{U_{#rho_{1}#sigma_{2}}^{0#beta}} #right]");
	l.DrawLatex(0.5, 0.600, "d#Gamma = #frac{1}{2m_{A}} #left( #prod_{f} #frac{d^{3}p_{f}}{(2#pi)^{3}} #frac{1}{2E_{f}} #right) #left| M #left(m_{A} - #left{p_{f}#right} #right) #right|^{2} (2#pi)^{4} #delta^{(4)} #left(p_{A} - #sum p_{f} #right)");
	...
	l.DrawLatex(0.5, 0.300, "p(n) = #frac{1}{#pi#sqrt{2}} #sum_{k = 1}^{#infty} #sqrt{k} A_{k}(n) #frac{d}{dn} #frac{sinh #left{ #frac{#pi}{k} #sqrt{#frac{2}{3}} #sqrt{n - #frac{1}{24}} #right}}{#sqrt{n - #frac{1}{24}}}");
#endif
	c1->Print("c1.gif");
	c1->Print("c1.jpg");
	c1->Print("c1.png");
