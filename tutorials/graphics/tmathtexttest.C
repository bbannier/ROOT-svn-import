void tmathtexttest(void)
{
	...
	l.DrawMathText(0.5, 0.3, "p(n) = {1\\over\\pi\\sqrt{2}} \\sum_{k = 1}^\\infty \\sqrt{k} A_k(n) {d\\over dn} {\\sinh \\left\\{ {\\pi\\over k} \\sqrt{2\\over 3} \\sqrt{n - {1\\over 24}} \\right\\} \\over \\sqrt{n - {1\\over 24}}}");
	l.DrawMathText(0.5, 0.1, "\\hbox{RHIC スピン物理}");
#else
	TLatex l;
	l.SetTextAlign(23);
	l.SetTextSize(0.06);
...
#endif
//	c1->Print("mathtext2.png");
//	c1->Print("mathtext2.ps");
}
