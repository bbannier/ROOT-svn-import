...
TMinuit2TraceObject::TMinuit2TraceObject(int parNumber) : ROOT::Minuit2::MnTraceObject(parNumber), 
   ...
   fIterOffset(0), 
   fHistoFval(0), fHistoEdm(0), fHistoParList(0), 
   fOldPad(0), fMinuitPad(0)
{
}
...
TMinuit2TraceObject::~TMinuit2TraceObject() {
   ...
   if (fHistoParList) { 
      for (int i=0; i < fHistoParList->GetSize(); ++i) { 
         ...
         if (h1) h1->GetXaxis()->SetRange(1,niter);
      }
   }
}
...
void TMinuit2TraceObject::Init(const ROOT::Minuit2::MnUserParameterState & state) {
   ...
   if (fHistoFval) delete fHistoFval; 
   if (fHistoEdm) delete fHistoEdm; 
   if (fHistoParList) { fHistoParList->Delete(); delete fHistoParList; }
   if (fMinuitPad) delete fMinuitPad;
   ...
   fHistoFval->SetCanRebin(kTRUE);
   fHistoEdm->SetCanRebin(kTRUE);
   ...
   for (unsigned int ipar = 0; ipar < state.Params().size(); ++ipar) { 
      ...
      h1->SetCanRebin(kTRUE);
      fHistoParList->Add(h1);      
   }



   if (gPad) fOldPad = gPad; 
