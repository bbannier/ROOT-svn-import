   ...
   fNMissed(0)
{
   ...
   //fMutexCond      = new TMutex();
   //fMutexSem       = new TMutex();
   //fUnzipCondition = new TCondition(fMutexCond);
   //fBufferCond     = new TCondition(fMutexSem);
...
TTreeCache::~TTreeCache()
{
 ...
 if (fBrNames) {fBrNames->Delete(); delete fBrNames; fBrNames=0;}
   ...
   if (fUnzipCondition) { delete fUnzipCondition;  fUnzipCondition = 0;}
   //if (fMutexCond) { delete fMutexCond;  fMutexCond = 0;}

   if (fUnzipBuffer) { delete [] fUnzipBuffer; fUnzipBuffer = 0;}
   if (fTmpBuffer  ) { delete [] fTmpBuffer;   fTmpBuffer = 0;}

   if (fUnzipLen)    { delete [] fUnzipLen; fUnzipLen = 0; }
   if (fUnzipPos)    { delete [] fUnzipPos; fUnzipPos = 0; } 

   if (fMutexCache) { delete fMutexCache; fMutexCache = 0;}
   if (fMutexUnzipBuffer) { delete fMutexUnzipBuffer; fMutexUnzipBuffer = 0;}
   if (fMutexBuffer     ) { delete fMutexBuffer; fMutexBuffer = 0;}
   if (fMutexList       ) { delete fMutexList; fMutexList = 0;}
   
   if (fBufferCond       ) { delete fBufferCond; fBufferCond = 0;}
   //if (fMutexSem) { delete fMutexSem;  fMutexSem = 0;}
}
...
Int_t TTreeCache::StartThreadUnzip()
{
   ...
   if(!fUnzipThread){
      fActiveThread=kTRUE;
      fUnzipThread= new TThread("UnzipLoop", UnzipLoop,
                                (void*) this);
      fUnzipThread->Run();
      return 0;
   }
...
Int_t TTreeCache::GetRecordHeader(char *buf, Int_t maxbytes, Int_t &nbytes, Int_t &objlen, Int_t &keylen)
{
   ...
   // That must be the pointer tho teh header part not the object by itself and
...
void TTreeCache::SetBufferRead(Long64_t pos, Int_t len, TBasket *basket) 
{
   ...
   while ((ind = (TBufferInfo*)next())) {
      ...
      if ((ind->GetPos()) == pos && (ind->GetBasket()) == basket){
         ...
         ind->SetRead(kTRUE);
         // Instead of marking as read.. just delete it from the list
         /*
...
Int_t TTreeCache::UnzipCache()
{
   ...
   for (Int_t i = localLastPos; i<localNseek; i++) { 
      // Always fro a change before continuing
      ...
      //Info("UnzipCache", "before fMutexUnzipBuffer->Lock()");
      ...
      //Info("UnzipCache", "before fMutexUnzipBuffer->UnLock()");
      ...
      // I know this is very very ugly but I need the scope for the mutex
      if(kTRUE){
         R__LOCKGUARD(fMutexList);
         TBufferInfo *ind = 0;
         TIter next(fUnzipList);
