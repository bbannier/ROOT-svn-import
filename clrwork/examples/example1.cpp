...
const int kRaisedFrame = 1;
const int kDoubleBorder = 1;
...
public:
   Option_t fOptions;
   void Emit(const char * c, bool on = false);
};
...
public:
   ...
   virtual void         Toggle(Bool_t emit = kFALSE) {
      SetDown(IsDown() ? kFALSE : kTRUE, emit);
   }
...
public:
   ...
   void               ChangeText(const char *title)  {
      SetTitle(title);   //*MENU*icon=bld_rename.png*
   }
...
public:
   ...
   virtual Bool_t IsDisabledAndSelected() const {
      return kButtonDisabled && fStateOn;
   }
...
public:
   ...
   virtual void MBPressed()  {
      Emit("MBPressed()");   // *SIGNAL*
   }
   virtual void MBReleased() {
      Emit("MBReleased()");   // *SIGNAL*
   }
   virtual void MBClicked()  {
      Emit("MBClicked()");   // *SIGNAL*
   }
   virtual void ItemClicked(Int_t id) {
      Emit("ItemClicked(Int_t)", id);   // *SIGNAL*
   }
