...
int testGAMinimizer() {
   ...
   ROOT::Math::GAlibMinimizer gaParabole;
   Parabole parabole;
   gaParabole.SetFunction(parabole);
   gaParabole.SetLimitedVariable(0, "x", 0, 0, -5, +5);
   gaParabole.SetGAScaling(ROOT::Math::GAlibMinimizer::GAlibSigmaTruncationScaling, 2.0);
   gaParabole.Minimize();
   cout << "Parabole min:" << gaParabole.MinValue() << endl;

   ROOT::Math::GAlibMinimizer gaRosenBrock;
   RosenBrockFunction RosenBrock;
   gaRosenBrock.SetFunction(RosenBrock);
   gaRosenBrock.SetLimitedVariable(0, "x", 0, 0, -5, +5);
   gaRosenBrock.SetLimitedVariable(0, "x", 0, 0, -5, +5);
   //gaRosenBrock.SetGAScaling(ROOT::Math::GAlibMinimizer::GAlibSigmaTruncationScaling, 2.0);
   gaRosenBrock.SetGAScaling(ROOT::Math::GAlibMinimizer::GAlibLinearScaling, 2.0);
   gaRosenBrock.SetGAType(ROOT::Math::GAlibMinimizer::GAlibTypeIncremental);
   gaRosenBrock.SetProperty("population_size", 300);
   gaRosenBrock.SetProperty("generations_to_convergence", 40);
   gaRosenBrock.Minimize();
   const double * xmin = gaRosenBrock.X(); 
   cout << "RosenBrock min: [" << xmin[0] << "] [" << xmin[1] << "]" << endl;

   ROOT::Math::GAlibMinimizer gaMultiMin;
   MultiMin multimin;
   gaMultiMin.SetFunction(multimin);
   gaMultiMin.SetLimitedVariable(0, "x", 0, 0, -5, +5);
   gaMultiMin.SetGAScaling(ROOT::Math::GAlibMinimizer::GAlibSigmaTruncationScaling, 2.0);
   gaMultiMin.SetGAVerbose(ROOT::Math::GAlibMinimizer::GAlibInitialPop);
   gaMultiMin.Minimize();
