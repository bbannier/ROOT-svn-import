...
namespace TMVA {
   ...
   private:
      Float_t  fXdiv;                  // Factor for division
      Int_t    fBest;                  // Best Edge for division
      //--- Integrals of all kinds ---
      Float_t  fVolume;                // Cartesian Volume of cell
      Float_t  fIntegral;              // Integral over cell (estimate from exploration)
      Float_t  fDrive;                 // Driver  integral, only for cell build-up
   ...
   public:
      ...
      //--------------- Geometry ----------------------------------
      Float_t   GetXdiv() const { return fXdiv;}          // Pointer to Xdiv
      Int_t     GetBest() const { return fBest;}          // Pointer to Best
      void      SetBest(Int_t    Best){ fBest =Best;}     // Set Best edge candidate
      void      SetXdiv(Float_t Xdiv){ fXdiv =Xdiv;}      // Set x-division for best edge cand.
      ...
      Float_t   GetVolume() const { return fVolume;}      // Volume of cell
      Float_t   GetIntg() const { return fIntegral;}      // Get Integral
      Float_t   GetDriv() const { return fDrive;}         // Get Drive
      void      SetIntg(Float_t Intg){ fIntegral=Intg;}   // Set true integral
      void      SetDriv(Float_t Driv){ fDrive   =Driv;}   // Set driver integral
      //--------------- linked tree organization ------------------
