...
cacheutils::CachingPdf::CachingPdf(RooAbsReal *pdf, const RooArgSet *obs) :
    ...
    pdfOriginal_(pdf),
    pdfPieces_(),
    pdf_(RooStats::Utils::FullCloneFunc(pdf, pdfPieces_)),
    lastData_(0),
    cache_(*pdf_,*obs_)
{
}
...
cacheutils::CachingPdf::CachingPdf(const CachingPdf &other) :
    ...
    pdfPieces_(),
    pdf_(RooStats::Utils::FullCloneFunc(pdfOriginal_, pdfPieces_)),
    lastData_(0),
    cache_(*pdf_,*obs_)
{
}
    ...
    if ((addpdf = dynamic_cast<RooAddPdf *>(pdf_)) != 0) {
        ...
        for (int i = 0; i < npdf; ++i) {
            ...
            if (0 && typeid(*funci) == typeid(RooProduct)) {
                ...
                obsInd.add(*coeff);
                RooStats::Utils::FactorizeFunc(*obs, *funci, obsDep, obsInd);
...
//LM    noDeepLEE_ = runtimedef::get("SIMNLL_NO_LEE");
   noDeepLEE_ = false; //LM
    ...
    factorizedPdf_.reset(dynamic_cast<RooSimultaneous *>(RooStats::Utils::FactorizePdf(*dataOriginal_->get(), *pdfclone, constraints)));
    
    RooSimultaneous *simpdf = factorizedPdf_.get();
    constrainPdfs_.clear(); 
    if (constraints.getSize()) {
//LM        bool FastConstraints = runtimedef::get("SIMNLL_FASTGAUSS");
       bool FastConstraints = false;
