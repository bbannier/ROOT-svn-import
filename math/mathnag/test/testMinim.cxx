...
TrigoFletcherFunction * fletcherFunc; 
...
int DoNewMinimization( const ROOT::Math::IMultiGenFunction & func, const double * x0, const double * s0, ROOT::Math::Minimizer * min, double &minval, double &edm, double * minx) { 
   ...
   ROOT::Math::MultiNumGradFunction * gfunc = new ROOT::Math::MultiNumGradFunction(func);
...
int DoOldMinimization( FCN  func, TVirtualFitter * min, int ndim, const double * x0, const double * s0, double &minval, double &edm) { 
  ...
  // set parameters
   for (int i = 0; i < ndim; ++i) { 
      std::string name = "x" + ROOT::Math::Util::ToString(i); 
      min->SetParameter(i,name.c_str(),x0[i], s0[i],0,0);
   }
...
int testNewMinimizer( const ROOT::Math::IMultiGenFunction & func, const double * x0, const double * s0, const std::string & minimizer, const std::string & algoType) { 
   ...
   if (minimizer == "Nag") { 
      min = new ROOT::Math::NagMinimizer(func.NDim() );
      if (min == 0) { 
...
int testOldMinimizer( FCN  func,  const double * x0, const double * s0, const std::string & fitter, int n=25) { 
   ...
   for (int i = 0; i < gNmin; ++i) { 
      gNCall = 0;
      iret |= DoOldMinimization(func, min,n, x0,s0,minval,edm); 
   }      
...
int testRosenBrock() { 
   ...
   //double xRB[2] = { -100.,100};  // should be -1,1
   double xRB[2] = { 0.1,0.1};  // should be -1,1
   
   // minimize using Rosenbrock Funciton
#ifndef DEBUG
   gNmin = 1000;
#endif
   //iret |= testOldMinimizer(RosenBrock,xRB, s0, "Minuit",2);
   //   iret |= testOldMinimizer(RosenBrock,"Minuit2",2);
    
   RosenBrockFunction fRB;

   //iret |= testNewMinimizer(fRB,xRB,s0,"Minuit","");
   iret |= testNewMinimizer(fRB,xRB,s0,"Nag","");
...
int testTrigoFletcher() { 
   ...
   fletcherFunc = &fTrigo; // global pointer used by the free function
   iret |= testOldMinimizer(TrigoFletcher,xTrigo, sTrigo, "Minuit",nT);

   iret |= testNewMinimizer(fTrigo,xTrigo,sTrigo,"nag","");
