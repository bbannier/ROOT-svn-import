...
class StatDialogBDT {  
 ...
 public:
   ...
   StatDialogBDT( const TGWindow* p, TString wfile = "weights/TMVARegression_BDT.weights.xml", 
...
StatDialogBDT* StatDialogBDT::fThis = 0;
...
StatDialogBDT::StatDialogBDT( const TGWindow* p, TString wfile, TString methName, Int_t itree )
   : fMain( 0 ),
     fItree(itree),
     fNtrees(0),
     fCanvas(0),
...
void BDT_Reg( const TString& fin = "TMVAReg.root" )
{
   ...
   if (!dir) {
      cout << "*** Error in macro \"BDT_Reg.C\": cannot find directory \"Method_BDT\" in file: " << fin << endl;
      return;
   }
   ...
   while ((key = (TKey*)next())) {
      ...
      if (!mdir) {
         cout << "*** Error in macro \"BDT_Reg.C\": cannot find sub-directory: " << key->GetName() 
      ...
      if (!strPath || !strWFile) {
         cout << "*** Error in macro \"BDT_Reg.C\": could not find TObjStrings \"TrainingPath\" and/or \"WeightFileName\" *** " << endl;
   ...
   for (UInt_t im=0; im<path.size(); im++) {
      ...
      fname += wfile[im];
      TString macro = Form( ".x BDT_Reg.C+\(0,\"%s\",\"%s\")", fname.Data(), methname[im].Data() );
