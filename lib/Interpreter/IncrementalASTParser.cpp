...
cling::IncrementalASTParser::IncrementalASTParser(clang::CompilerInstance* CI,
  ...
  Ctx->PrintingPolicy.Dump = 1;
  Ctx->PrintingPolicy.SuppressSpecifiers = 0;
  Ctx->PrintingPolicy.SuppressTag = 0;
  Ctx->PrintingPolicy.SuppressScope = 0;
...
void cling::IncrementalASTParser::handlePendingSemaActions() {
  // Sort of like Sema::ActOnEndOfTranslationUnit()
  clang::Scope* bakScope = m_Sema->TUScope;
  llvm::SmallVector<const DeclaratorDecl*, 4> bakUnusedFileScopedDecls(m_Sema->UnusedFileScopedDecls);
  // m_Sema->UnusedFileScopedDecls.clear();
  m_Sema->CompleteTranslationUnit = true;

  m_Sema->ActOnEndOfTranslationUnit();

  m_Sema->TUScope = bakScope;
  //m_Sema->UnusedFileScopedDecls = bakUnusedFileScopedDecls;
}
