...
e = Consumers.end(); i != e; ++i) (*i)->WHAT PARAM; \
  ...
  class ASTPragmaHandler: public clang::PragmaHandler {
  public:
    ASTPragmaHandler(IncrementalASTParser* IAP):
    clang::PragmaHandler("ast"), m_IAP(IAP) {}
    virtual ~ASTPragmaHandler() {}
    
    void HandlePragma(clang::Preprocessor &PP,
                      clang::PragmaIntroducerKind Introducer,
                      clang::Token &Tok) {
      PP.LexUnexpandedToken(Tok);
      assert(Tok.is(clang::tok::identifier) && "Token after \"#pragma cling ast\" must be identifier!");
      const clang::IdentifierInfo *II = Tok.getIdentifierInfo();
      assert(II->isStr("end_of_input") && "only supported \"#pragma cling ast\" option is \"end_of_input\"");
      m_IAP->RequestParseInterrupt(Tok.getLocation());
    }
    
  private:
    IncrementalASTParser* m_IAP; // "our" incremental AST parser
  };
} // namespace cling
...
m_Consumer(0) {
  ...
  Pragma->AddPragma(new ASTPragmaHandler(this)); // owned by Pragma
...
cling::IncrementalASTParser::parse(llvm::StringRef src,
  ...
  m_MemoryBuffer->append("\n#pragma cling ast end_of_input\n");
    ...
    if (ADecl) {
      ...
      if (m_InterruptHere.isValid()) {
        ...
        if (DGR.isSingleDecl()) {
          ...
          size_t NDecl = DGR.getDeclGroup().size();
          if (NDecl > 0) {
            D = DGR.getDeclGroup()[NDecl - 1];
          }
        }
        if (D) {
          clang::SourceLocation Loc = D->getLocEnd();
          if (!Loc.isValid()) Loc = D->getLocStart();
          if (Loc.isValid() && !(Loc < m_InterruptHere)) {
            break;
          }
        }
  ...
  if (PP.LookAhead(0).is(clang::tok::eof))
