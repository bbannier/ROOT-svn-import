...
\section{Fundamental Interfaces in RooStats}
...
In RooFit even simple real-valued parameters and observables are treated as a C++ objects, instead of just a simple floating point number.  This allows the variable to carry around a name, units, upper- and lower- values for fitting and integration, etc.  For real-valued variables, the class is \textbf{RooRealVar}, but there are equivalent classes for discrete categories etc.  They all inherit from an interface called \textbf{RooAbsArg} (for abstract argument).  

In addition to real-valued variables, one can define real-valued functions, both of which inherit from \textbf{RooAbsReal} (for abstract real-valued thingy).  The most important methods in the interface are listed below.

\lstinputlisting{code_snippits/RooAbsReal.h}

One can create sets and lists of these abstract arguments called \textbf{RooArgSet} and \textbf{RooArgList}, respectively.  They are often used to represent a list of parameters or observables.
...
\[
  G(x|\mu, \sigma)
\]
In RooFit / RooStats code, one an construct this via:
\lstinputlisting[firstline=2,lastline=11]{GaussianModel.C}
or via
\lstinputlisting[firstline=2,lastline=5]{GaussianModel_factory.C}
...
\lstinputlisting{code_snippits/RooWorkspace.h}



\textbf{ModelConfig} is a simple class that holds configuration information specifying how a model should be used in the context of various RooStats tools.  A single model can be used in different ways, and this class should carry all that is needed to specify how it should be used.  In particular, it can identify which parameters are parameters of interest and which are nuisance parameters.  It can also identify which PDFs are part of the likelihood function and which are prior PDFs.

\lstinputlisting{code_snippits/ModelConfig.h}
...
\lstinputlisting{code_snippits/ConfInterval.h}
...
\lstinputlisting{code_snippits/IntervalCalculator.h}
...
\lstinputlisting{code_snippits/HypoTestResult.h}
...
\lstinputlisting{code_snippits/HypoTestCalculator.h}


\subsection{Test Statistic, Sampling Distribution}

See the section devoted to Test Statistic, Sampling Distribution.
...
The approximate frequentist method based on Wilks' theorem.  See section~\ref{S:ProfileLikelihoodRatio}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	\subsection{Profile Likelihood Ratio (the method of MINOS)}\label{S:ProfileLikelihoodRatio}
...
\lstinputlisting{code_snippits/RooStatsUtils.h}
...
\lstinputlisting{code_snippits/NumberCountingUtils.h}
