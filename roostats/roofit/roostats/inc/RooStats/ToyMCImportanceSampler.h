...
/*
      using ToyMCSampler::SetPdf;
      virtual void SetPdf(RooAbsPdf& pdf) { 
         ToyMCSampler::SetPdf(pdf);
         if( fNullDensities.size() == 0 ) {
            oocoutI((TObject*)0,InputArguments) << "Pdf specified using ToyMCSampler::SetPdf() will also be added to the null densities for importance sampling." << endl;
            AddNullDensity( &pdf );
         }
      }
      using ToyMCSampler::SetParametersForTestStat;
      virtual void SetParametersForTestStat(const RooArgSet& nullpoi) {
         ToyMCSampler::SetParametersForTestStat( nullpoi );
         if( fPdf && fNullSnapshots.size() == 1  &&  fNullSnapshots[0] == NULL) {
            oocoutI((TObject*)0,InputArguments) << "Snapshot specified using ToyMCSampler::SetParametersForTestStat(). This will also be set as the snapshot for the only null density currently specified and which does not have a snapshot yet." << endl;
            fNullSnapshots.clear();
            fNullSnapshots.push_back( &nullpoi );
         }
      }
*/

      void SetToysStrategy( toysStrategies t ) { fToysStrategy = t; }
