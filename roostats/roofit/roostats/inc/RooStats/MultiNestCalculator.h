// @(#)root/roostats:$Id: MultiNestCalculator.h 40959 2011-09-20 16:33:21Z moneta $
...
#define ROOSTATS_MultiNestCalculator
...
namespace RooStats {
   ...
   typedef MCMCInterval MultiNestInterval;
   typedef MCMCIntervalPlot MultiNestIntervalPlot;
   typedef MarkovChain PosteriorDistribution;

   class ModelConfig;

   class MultiNestCalculator : public IntervalCalculator, public TNamed {
   ...
   public:
      ...
      // ModelConfig. Any of these basic settings can
      // be overridden by calling one of the Set...() methods.
      MultiNestCalculator(RooAbsData& data, const ModelConfig& model);

      virtual ~MultiNestCalculator() {}

      // Main interface to get a ConfInterval
      virtual MultiNestInterval* GetInterval() const;
      ...
      virtual void SetParametersOfInterest(const RooArgSet& set) { SetParameters(set); }

      // specify the parameters to store in the posterior
      // By default all the parameters are stored 
      virtual void SetPosteriorParameters(const RooArgSet & set) { fPosteriorParams.removeAll(); fPosteriorParams.add(set); }
...
*/
      ...
      /// set constant efficiency mode
      virtual void Ceff(Bool_t ceff) { fCeff = ceff; }
      /// switch on/off multi-modal sampling
      virtual void MModal(Bool_t mmodal) { fMModal = mmodal; }
      /// set tolerance
      virtual void Tol(Double_t tol) { fTol = tol; }
      /// set target efficiency for "constant efficiency mode"
      virtual void Efr(Double_t efr) { fEfr = efr; }
      /// set maximum number of modes
      virtual void MaxModes(Int_t maxModes) { fMaxModes = maxModes; }
      /// set number of live points
      virtual void Nlive(Int_t nlive) { fNlive = nlive; }
   ...
   protected:
      ...
      RooArgSet   fPosteriorParams; // parameters to store in the chain (if not specified they are all of them )
      ...
      enum MultiNestInterval::IntervalType fIntervalType; // type of interval to find
      ...
      ClassDef(MultiNestCalculator,1) // multi mode nested sampling calculator for Bayesian credible intervals
   };
}
