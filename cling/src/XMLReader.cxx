...
bool XMLReader::GetAttributes(const std::string& tag, std::vector<Attributes>& out)
{
  ...
  if (attrstr.length() > 4) { //ELSE ERROR HANDLING; - no need for it - I check in Parse()
    ...
    for (std::string::size_type i = 0, e = attrstr.length()-1; i < e; ++i) {
      ...
      if (c == '=') {
	if (!name){ // if no name was read, report error (i.e. <class ="x">)
	...
	return false;
      ...
      else if ((newattr || name) && !value){ // else - if name or newattr is set, we should write in the attr_name variable
	newattr = false;
	name = true;
	attr_name += c;
	lastsymbol = c;
    ...
    if (name && (!equalfound || !value)) { // this catches the situation <class name = "value" something >
...
bool XMLReader::Parse(std::ifstream &file, SelectionRules& out)
{
  ...
  while(file.good()){
	...
	vsr = new VariableSelectionRule(); // the field is variable selection rule object
	bsrChild = vsr;
	break;
      case kMethod:
	if (parent.empty()){ // if we have a <field>, <method> or <properties> tag outside a parent <clas>s tag, 
	...
	fsr = new FunctionSelectionRule(); // the method is function selection rule object
	bsrChild = fsr;
	break;
      case kProperties:
	if (parent.empty()){ // if we have a <field>, <method> or <properties> tag outside a parent <clas>s tag, 
	...
	break;
      case kFunction: 
	fsr = new FunctionSelectionRule();
	bsr = fsr;
	break;
      case kVariable: 
	vsr = new VariableSelectionRule();
	bsr = vsr;
	break;
      case kEnum:
	esr = new EnumSelectionRule();
	bsr = esr;
	break;
      case kLcgdict: 
      case kEndLcgdict: 
	...
	if (!exclusion && !IsClosingTag(tagStr)) { // exclusion should be false, we are not interested in closing tags
	  ...
	  if (tagKind == kLcgdict || tagKind == kSelection) 
	    ...
	    if (tagKind == kField || tagKind == kMethod) bsrChild->setSelected(kYes); // if kMethod or kField - add to child
	    else bsr->setSelected(kYes); 
	  }
	}
	else { // if exclusion = true
	  if (IsStandaloneTag(tagStr)){
	    // DEBUG std::cout<<"No"<<std::endl;
	    if (tagKind == kField || tagKind == kMethod) bsrChild->setSelected(kNo);
	    else bsr->setSelected(kNo);
	  }
	  ...
	  // rule should be No. So for the parent class it is - Don't care; for the children it is No
	    bsr->setSelected(kDontCare); // this is for the parent
	  }
	...
	if (attr.empty()) ;// DEBUG std::cout<<"Tag doesn't have attributes"<<std::endl<<std::endl;
	  ...
	  for (int i = 0, n = attr.size(); i < n; ++i) {
	    // DEBUG std::cout << "\tAttrName[" << i << "]: " << attr[i].name << " | AttrValue["<<i<<"]: "<<attr[i].value<<std::endl;
	    ...
	    if (tagKind == kClass || tagKind == kProperties || tagKind == kEnum || tagKind == kFunction || 
	       tagKind == kVariable) {
               ...
               bsr->setAttributeValue(attr[i].name, attr[i].value);
               if ((attr[i].name == "file_name" || attr[i].name == "file_pattern") && tagKind == kClass){
                  bsr->setAttributeValue("pattern","*");
                  out.setHasFileNameRule(true);
               }
	    }
	    else {
               if (bsrChild->hasAttributeWithName(attr[i].name)) {
                  std::cout<<"Error - duplicating attribute at line "<<lineNum<<std::endl;
                  out.clearSelectionRules();
                  return false;
               }
               bsrChild->setAttributeValue(attr[i].name, attr[i].value);
	    }
	  }
