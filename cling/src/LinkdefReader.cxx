...
bool LinkdefReader::CPPHandler(std::ifstream& file, SelectionRules& sr)
{
   ...
   while (file.good()){
      ...
      case kIfdef:
         {
            ...
            if (next != "__CINT__" && next != "__MAKECINT__") { // process only __CINT__ and __MAKECINT__ statements
               std::cout<<"Error at line "<<line<<" - wrong ifdef argument"<<std::endl;
               sr.clearSelectionRules();
               return false;
            }
            if (haveMoreTokens) {
               std::cout<<"Warning at line "<<line<<" - too many arguments of ifdef statement"<<std::endl;
    ...
    case kEndif:
      ...
      GetNextToken(file, next, true);
      if (!next.empty()) {
	std::cout<<"Warning at line "<<line-1<<" - too many arguments of endif statement"<<std::endl;
      }
	...
	bool haveMoreTokens = GetNextToken(file, next, false);
	if (haveMoreTokens) {
	  std::cout<<"Error at line "<<line<<" - too many if arguments"<<std::endl;
	  sr.clearSelectionRules();
	  return false;
	}
	if (next != "0" && next != "1") {
	  std::cout<<"Warning at line "<<line<<" - possibly inimplemented if statement"<<std::endl;
	  break;
	}
    ...
    case kElse:
      ...
      GetNextToken(file, next, true);
      if (!next.empty()) {
	std::cout<<"Warning at line "<<line-1<<" - too many arguments of else statement"<<std::endl;
      }
      inside_if = false;
      inside_else = true;
      break;
    case kUnrecognized:
      ...
      std::cout<<"Error at line "<<line<<" - unrecognized pragma statement"<<std::endl;
      sr.clearSelectionRules();
      return false;
    }
...
bool LinkdefReader::PragmaParser(std::ifstream& file, SelectionRules& sr)
{
  ...
  if(!(GetNextToken(file, link_token, false))) {
    std::cout<<"Warning at line "<<line-1<<" - lonely pragma statement"<<std::endl;
    return true;
  }
  
  if (link_token != "link") {
    std::cout<<"Warning at line "<<line<<" - first pragma option isn't link - this pragma does nothing here"<<std::endl;
  ...
  if (!GetNextToken(file, on_off_token, false)) {
    std::cout<<"Warning at line "<<line-1<<" - incomplete pragma statement"<<std::endl;
    return true;
  }
  ...
  if (on_off_token == "off") linkOn = false;
    ...
    std::cout<<"Error at line "<<line<<" - bad #pragma format"<<std::endl;
    return false;
  }
  ...
  case kAll:
	...
	std::cout<<"Error at line "<<line<<" - missing ; at end of line"<<std::endl;
	return false;
      }
      std::cout<<"Warning at line "<<line<<" - this pragme statement is incomplete and does nothing"<<std::endl;
      break;
    }
	...
	std::cout<<"Error at line "<<line-1<<" - missing ; at end of line"<<std::endl;
	return false;
      }
    }
    ...
    if(rule_token == "globals"){
      ...
      //else vsr.setSelected(kNo);
      //sr.addVariableSelectionRule(vsr);

      EnumSelectionRule esr;
      if (linkOn) {
         esr.setSelected(kYes);
      ...
      std::cout<<"Warning at line "<<line<<" - possibly unimplemented pragma statement"<<std::endl;
    }
  ...
  case kNestedclasses:
	...
	if (!temp_token.empty()) {
	  std::cout<<"Warning at line "<<line-1<<" - too much pragma arguments (will work all the same)"<<std::endl;
	}
      }
	...
	TrimChars(temp_token, " \t\n;");
	if (!temp_token.empty()) {
	  std::cout<<"Warning at line "<<line-1<<" - too much stuff after nestedclasses"<<std::endl;
	}
      }
    }
    break;
  case kDefinedIn:
	...
	std::cout<<"Warning at line "<<line-1<<" - unfinished pragma statement"<<std::endl;
	break;
      }
	...
	return false;
      ...
      if (linkOn) vsr.setSelected(kYes);
      else vsr.setSelected(kNo);
      sr.addVariableSelectionRule(vsr);
      ...
      if (linkOn) esr.setSelected(kYes);
      else esr.setSelected(kNo);
      sr.addEnumSelectionRule(esr);
    
      FunctionSelectionRule fsr;
      fsr.setAttributeValue("pattern","*");
      fsr.setAttributeValue("file_name",rule_token);
      if (linkOn) fsr.setSelected(kYes);
      else fsr.setSelected(kNo);
      sr.addFunctionSelectionRule(fsr);
  ...
  case kStruct:
	...
	return false;
      ...
      std::cout<<"Warning at line "<<line-1<<" - incomplete pragma statement - it does nothing"<<std::endl;
      break;
    }
    ...
    break;
  case kUnknown:
    std::cout<<"Warning at line "<<line<<" - unimplemented pragma statement - it does nothing"<<std::endl;
    if (haveMoreTokens) {
      GetNextToken(file, temp_token, true);
    }
    break;
...
bool LinkdefReader::ProcessFunctionPrototype(std::string& proto, bool& name)
{
  ...
  if (pos1 != pos1_1) {
    std::cout<<"Error at line "<<line<<" - too many ( in function prototype!"<<std::endl;
    return false;
  }
  ...
  if (pos1 > -1){
    if (pos2 < 0) {
      std::cout<<"Error at line "<<line<<" - missing ) in function prototype"<<std::endl;
      return false;
    }
    if (pos2 < pos1) {
      std::cout<<"Error at line "<<line<<" - wrong order of ( and ) in function prototype"<<std::endl;
      return false;
    }
...
bool LinkdefReader::ProcessOperators(std::string& pattern)
{
  ...
  while (true) {
    ...
    if (!ok1 && !ok2) {
      std::cout<<"Error at line "<<line-1<<" - extra space"<<std::endl;
      return false;
    }
    pattern.erase(pos, 1);
  }
  ...
  if (open_br != close_br) {
    std::cout<<"Error at line "<<line<<" - number of < doesn't match number of >"<<std::endl;
    return false;
  }
...
bool LinkdefReader::RemoveComment(std::ifstream& file)
{
  ...
  if (file.good())
    ...
    std::cout<<"Error at line "<<line<<" - unexpected end of file"<<std::endl;
    return false;
  }
  ...
  if (c == '/') { // remove // comments
	...
	std::cout<<"Error at line "<<line<<" - file ended before end of comment was reached"<<std::endl;
	return false;
      }
    }
    ...
    std::cout<<"Error at line "<<line<<" - this is not a comment"<<std::endl;
    return false;
  }
  return true;
}
      ...
      if (file.good()) {
         ...
         if (c == '/'){
            ...
            if (c2 == '/' || c2 == '*') { // if comment - remove comment
               ...
               std::cout<<"Error at line "<<line<<" - lonely / in the begining of the line"<<std::endl;
               token = "";
               return false;
            }
         }
            ...
            while (file.good()) {
               ...
               if (file.good()) {
                  if (c2 == '\n') {
                     //std::cout<<"\t\t++line"<<std::endl;
                     ++line;
                     std::cout<<"Error at line "<<line<<" - bad statement"<<std::endl;
                     token = "";
                     return false;
                  }
                  else if (c2 != ' ') break;
         ...
         // something different than space, comment or #something at the beginning of a line - Error
            ...
            std::cout<<"Error "<<line<<" - unrecognized statement"<<std::endl;
            token = "";
            return false;
         }
      }
