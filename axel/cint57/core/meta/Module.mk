...
METATCINT7MAP  := $(METATCINTLIB:.$(SOEXT)=.rootmap)
...
$(METATCINTLIB): $(METATCINTO) $(METATCINTDO) $(ORDER_) $(CINTLIB) $(METATCINTLIBDEP)
		@$(MAKELIB) $(PLATFORM) $(LD) "$(LDFLAGS)" \
		   "$(SOFLAGS)" libMetaTCint.$(SOEXT) $@ "$(METATCINTO) $(METATCINTDO)" \
		   "$(METATCINTLIBEXTRA)"

ifneq ($(BUILDCINT7),)
$(METATCINT7LIB): $(METATCINT7O) $(METATCINT7DO) $(ORDER_) $(CINT7LIB) $(METATCINT7LIBDEP)
		@$(MAKELIB) $(PLATFORM) $(LD) "$(LDFLAGS)" \
		   "$(SOFLAGS)" libMetaTCint_7.$(SOEXT) $@ "$(METATCINT7O) $(METATCINT7DO)" \
		   "$(METATCINT7LIBEXTRA)"
endif
...
all-$(MODNAME): $(METAO) $(METADO) $(METATCINTLIB) $(METATCINT7LIB) $(METATCINTMAP) $(METATCINT7MAP)

clean-$(MODNAME):
		@rm -f $(METAO) $(METADO) $(METATCINTO) $(METATCINTDO) $(METATCINT7O) $(METATCINT7DO)
