...
//
// Root > T->Process("TPosCorr.C")
// Root > T->Process("TPosCorr.C","some options")
// Root > T->Process("TPosCorr.C+")
//
...
void TPosCorr::SlaveBegin(TTree * /*tree*/)
{
   ...
   fHistCorrPos = new TH2F("hPosCorr", "corrected XY position;X position;Y position",
                          100, -10., 10., 100, -10., 10.);
...
Bool_t TPosCorr::Process(Long64_t entry)
{
   ...
   // It can be passed to either TPosCorr::GetEntry() or TBranch::GetEntry()
   ...
   for (Int_t i = 0; i < electrons_; ++i) {
      ...
      fHistCorrPos->Fill(electrons_fVertex_fX[i] - 1.2, electrons_fVertex_fY[i] - 0.9);
   }

   return kTRUE;
}

void TPosCorr::SlaveTerminate()
{
