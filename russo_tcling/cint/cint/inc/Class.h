...
public:
   ...
   G__ClassInfo(): tagnum(0), class_property(0) {
      Init();
   }
   void Init();
   G__ClassInfo(const char* classname): tagnum(0), class_property(0) {
      Init(classname);
   }
   void Init(const char* classname);
   G__ClassInfo(const G__value& value_for_type);
   G__ClassInfo(int tagnumin): tagnum(0), class_property(0) {
      Init(tagnumin);
   }
   ...
   int IsValid() {
      return 0 <= tagnum && tagnum < G__struct.alltag ? 1 : 0;
   }
#else
   int IsValid();
#endif
