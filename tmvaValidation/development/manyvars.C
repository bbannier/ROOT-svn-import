// @(#)root/tmva $Id: manyvars.C,v 1.1 2007-04-03 22:38:21 andreas.hoecker Exp $
/**********************************************************************************
...
void manyvars( TString myMethodList = "" ) 
{
   ...
   //
   // mylinux~> root -l manyvars.C\(\"myMethod1\ myMethod2\ myMethod3\"\)
   //
   ...
   if (mlist->GetSize()>0) {
      Use_CutsGA = Use_CutsD = Use_Cuts
         ...
         = Use_HMatrix = Use_Fisher = Use_BDTD = Use_BDT = Use_RuleFit = Use_SVM = 0;
      ...
      if (mlist->FindObject( "SVM"           ) != 0) Use_SVM           = 1; 

      delete mlist;
   }
  
   std::cout << "Start Test manyvars" << std::endl
        << "======================" << std::endl
        << std::endl;
   ...
   if (ReadDataFromAsciiIFormat) {
      ...
      TFile *input(0);
      if(!gSystem->AccessPathName("./data/manyvars.root")) {
         // first we try to find tmva_example.root in the local directory
         cout << "--- manyvars  : accessing ./data/manyvars.root" << endl;
         input = TFile::Open("./data/manyvars.root");
      } 
      else { 
         // second we try accessing the file via the web from
         // http://root.cern.ch/files/tmva_example.root
         cout << "--- manyvars  : accessing tmva_example.root file from http://root.cern.ch/files" << endl;
         cout << "--- manyvars  : for faster startup you may consider downloading it into you local directory" << endl;
   ...
   for (Int_t ivar=0; ivar<50; ivar++) factory->AddVariable(Form("var%i",ivar), 'F');
   ...
   std::cout << "==> manyvars is done!" << std::endl;      
