// @(#)root/tmva $Id: TMVAClassificationManyVars.C,v 1.1 2008-11-18 21:16:22 andreas.hoecker Exp $
/**********************************************************************************
 * Project   : TMVA - a Root-integrated toolkit for multivariate data analysis    *
 * Package   : TMVA                                                               *
 * Root Macro: TMVAClassificationManyVars                                         *
 *                                                                                *
 ...
 *                                                                                *
 *    root -l TMVAClassificationManyVars.C\(\"Fisher,Likelihood\"\)               *
 *                                                                                *
...
void TMVAClassificationManyVars( TString myMethodList = "" ) 
{
   ...
   //
   // mylinux~> root -l TMVAClassificationManyVars.C\(\"myMethod1,myMethod2,myMethod3\"\)
   //
   // if you like to use a method via the plugin mechanism, we recommend using
   // 
   // mylinux~> root -l TMVAClassificationManyVars.C\(\"P_myMethod\"\)
   ...
   if (myMethodList != "") {
      Use_CutsGA = Use_CutsSA = Use_CutsD = Use_CutsPCA = Use_Cuts
         ...
         = Use_HMatrix = Use_Fisher = Use_BDTD = Use_BDT = Use_LD
      ...
      if (mlist->FindObject( "LD"        		) != 0) Use_LD		        = 1; 
   ...
   std::cout << "Start TMVAClassificationManyVars" << std::endl
             << "=================" << std::endl
             << std::endl;
   ...
   TMVA::Factory *factory = new TMVA::Factory( "TMVAClassificationManyVars", outputFile, 
                                               Form("!V:!Silent:%sColor:Transformations=P,G,D", gROOT->IsBatch()?"!":"") );
   ...
   factory->AddVariable( "var5", 'F' );
   factory->AddVariable( "var6", 'F' );
   factory->AddVariable( "var7", 'F' );
   factory->AddVariable( "var8", 'F' );
   factory->AddVariable( "var9", 'F' );
   ...
   if (ReadDataFromAsciiIFormat) {
      ...
      TFile *input(0);
      TString fname = "./ManyVarsData.root";
      if (!gSystem->AccessPathName( fname )) {
         // first we try to find tmva_example.root in the local directory
         std::cout << "--- TMVAClassificationManyVars  : Accessing " << fname << std::endl;
         input = TFile::Open( fname );
      } 
      else { 
         // second we try accessing the file via the web from
         // http://root.cern.ch/files/tmva_example.root
         std::cout << "--- TMVAClassificationManyVars  : Accessing tmva_example.root file from http://root.cern.ch/files" << std::endl;
         std::cout << "--- TMVAClassificationManyVars  : For faster startup you may consider downloading it into you local directory" << std::endl;
      ...
      gROOT->cd( outfileName+TString(":/") ); // without this line there is a ROOT-error for some of the root-files because the output-file is not opened when internally (in the DataSetFactory the tree is copied with the given cut). 
   ...
   if (Use_PDEFoam)
      factory->BookMethod( TMVA::Types::kPDEFoam, "PDEFoam", 
                           "!H:V:SigBgSeparate=T:TailCut=0.001:VolFrac=0.0333:nCells=500:nSampl=2000:nBin=5:Compress=T:Kernel=None:CutNmin=F:Nmin=100" );
   ...
   // Fisher discriminant   
   if (Use_Fisher) 
      factory->BookMethod( TMVA::Types::kFisher, "Fisher", "H:!V:!Normalise" );

   // Linear discriminant (same as Fisher)
   if (Use_LD)
      factory->BookMethod( TMVA::Types::kLD, "LD", "H:!V:!Normalise" );
   ...
   if (Use_MLP)
      factory->BookMethod( TMVA::Types::kMLP, "MLP", "H:!V:!Normalise:NeuronType=tanh:NCycles=200:HiddenLayers=N+1,N:TestRate=5:BPMode=sequential:BatchSize=-1" );
   ...
   if (Use_SVM_Gauss)
      factory->BookMethod( TMVA::Types::kSVM, "SVM_Gauss", "Sigma=2:C=1:Tol=0.001:Kernel=Gauss:VarTransform=Norm" );

   if (Use_SVM_Poly)
      factory->BookMethod( TMVA::Types::kSVM, "SVM_Poly", "Order=4:Theta=1:C=0.1:Tol=0.001:Kernel=Polynomial:VarTransform=Norm" );

   if (Use_SVM_Lin)
      factory->BookMethod( TMVA::Types::kSVM, "SVM_Lin", "!H:!V:Kernel=Linear:C=1:Tol=0.001:VarTransform=Norm" );  
   ...
   std::cout << "==> TMVAClassificationManyVars is done!" << std::endl;      
