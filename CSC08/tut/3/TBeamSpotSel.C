...
void TBeamSpotSel::SlaveBegin(TTree * /*tree*/)
{
   ...
   GetOutputList()->Add(fBeamSpotX_Time);
   GetOutputList()->Add(fBeamSpotY_Time);
   GetOutputList()->Add(fBeamSpotZ_Time);
   GetOutputList()->Add(fBeamSpotXYZ);
}
...
void TBeamSpotSel::Terminate()
{
   ...
   TH1* h = (TH1*) GetOutputList()->FindObject("hBeamSpotXYZ");
   h->Draw();
}
