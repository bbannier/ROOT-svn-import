...
void read_tree()
{
   ...
   // open the Root file
   TFile *f = TFile::Open("http://localhost/ROOT/exercise2.root");
   if (!f || f->IsZombie()) {
      printf("Problem opening http://localhost/ROOT/exercise2.root \n");
      return;
   }
   ...
   myTree->SetBranchAddress("electrons", &particles);
