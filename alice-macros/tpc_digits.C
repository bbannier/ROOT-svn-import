// $Id: tpc_digits.C,v 1.9 2007/10/22 14:49:12 mtadel Exp $
...
void tpc_digits(Int_t mode=1)
{
  ...
  */
  ...
  case 1: { // Display all sectors
    gEve->DisableRedraw();
    {
      Reve::ElementList* l = new Reve::ElementList("TPC plate 1");
    ...
    {
      Reve::ElementList* l = new Reve::ElementList("TPC plate 2");
      l->SetTitle("TPC Plate");
      l->SetMainColor(Color_t(col));
...
void tpc_digits_2drange(Int_t start, Int_t end)
{
  ...
  gEve->DisableRedraw();
  {
    Reve::ElementList* l = new Reve::ElementList("TPC sectors");
    l->SetMainColor(Color_t(col));
    gEve->AddElement(l);
