void tof_digits_strips()
{
  ...
  Reve::ElementList* ll = new Reve::ElementList("TOF");
  ...
  for(Int_t iSector=0; iSector<g->NSectors(); iSector++) {
    ...
    Reve::ElementList* l = new Reve::ElementList(sectorName);
    ...
    for(Int_t iPlate=0; iPlate<g->NPlates(); iPlate++) {
      ...
      sPlate=bsPlate;
      sPlate+=iPlate;
      Reve::ElementList* relPlate = new Reve::ElementList(sPlate.Data());
