...
*/
BuiltinMap & el_internal_builtins()
{
...
void el_builtins_init() // internal func
{
...
void el_register_function( const char * name, el_builtin_t::handler_func f, const char * help )
{
...
/**
   ...
   state.
*/
int el_func_readline_reinit( EditLine * el, int, const char ** )
{
...
el_builtin_t * el_builtin_by_name( const char *name )
{
        BuiltinMap::iterator it = el_internal_builtins().find(name);
        BuiltinMap::iterator et = el_internal_builtins().end();
        if( et == it ) return NULL;
        return & (*it).second;
}
